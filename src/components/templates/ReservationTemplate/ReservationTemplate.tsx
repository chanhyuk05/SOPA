...
interface ReservationTemplateProps {
  user: User;
  onReservationCreate?: (reservation: Omit<Reservation, 'id' | 'createdAt'>) => void;
...
const ReservationTemplate: React.FC<ReservationTemplateProps> = ({
  ...
  const [musicRooms] = useState<MusicRoom[]>(MUSIC_ROOMS);
  const [timeSlots] = useState<TimeSlot[]>(DEFAULT_TIME_SLOTS);
  const [reservations, setReservations] = useState<Reservation[]>([]);
  
  // 모달 상태
  const [modalVisible, setModalVisible] = useState(false);
  const [selectedMusicRoom, setSelectedMusicRoom] = useState<MusicRoom | undefined>();
